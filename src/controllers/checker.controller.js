...
const searchStbj = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Gagal mencari data STBJ.' });
    }
};
...
const loadStbjData = async (req, res) => {
    try {
        ...
        // Query untuk mengambil data "Kirim" dari STBJ
        const stbjQuery = `
            SELECT 
            ...
            JOIN tspk_dc spk ON d.stbjd_spk_nomor = spk.spkd_nomor
            JOIN tbarangdc brg ON spk.spkd_kode = brg.brg_kode
            JOIN tbarangdc_dtl dtl ON brg.brg_kode = dtl.brgd_kode AND d.stbjd_size = dtl.brgd_ukuran
            WHERE d.stbjd_stbj_nomor = ?;
        `;
        const [stbjItems] = await pool.query(stbjQuery, [stbjNomor]);

        if (stbjItems.length === 0) {
            return res.status(404).json({ success: false, message: 'Data STBJ tidak ditemukan.' });
        }

        // Ambil semua nomor packing yang terkait
        const packingNomors = stbjItems.map(item => item.stbjd_packing);

        // Query untuk mengambil data "Terima" dari hasil packing
        const packingQuery = `
        ...
        `;
        const [packingItems] = await pool.query(packingQuery, [packingNomors]);
        
        // Buat map untuk lookup cepat
        const packingMap = new Map(packingItems.map(p => [p.packd_barcode, p.jumlahPacking]));
        
        // Gabungkan data STBJ dengan data Packing
        ...
        res.status(500).json({ success: false, message: 'Gagal memuat data STBJ.' });
    }
};
...
const onCheck = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: `STBJ ${stbj_nomor} telah divalidasi.` });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Gagal mengupdate status STBJ.' });
    }
};
