    ...
    const query = `
      SELECT x.*, v.divisi AS Divisi FROM (
        SELECT 
          ...
          IFNULL((SELECT COUNT(*) FROM kencanaprintnew.tgarmenrealisasi_hdr q WHERE q.re_minta=h.min_nomor),0) AS totr,
          IFNULL((SELECT COUNT(*) FROM kencanaprintnew.tgarmenrealisasi_hdr q WHERE q.re_minta=h.min_nomor AND q.re_apv IS NOT NULL),0) AS tota,
          IFNULL((
            SELECT IFNULL(IF(pin_acc="" AND pin_dipakai="","WAIT",
                   IF(pin_acc="Y" AND pin_dipakai="","ACC",
                   IF(pin_acc="Y" AND pin_dipakai="Y","",
                   IF(pin_acc="N","TOLAK","")))),"")
            FROM kencanaprintnew.tspk_pin5 WHERE pin_trs="PERMINTAAN GARMEN" AND pin_nomor=h.min_nomor ORDER BY pin_urut DESC LIMIT 1
          ),"") AS Ngedit
        FROM kencanaprintnew.tgarmenminta_hdr h
        LEFT JOIN kencanaprintnew.tgudangproduksi g ON g.gdgp_kode = h.min_gp
        LEFT JOIN kencanaprintnew.tspk s ON s.spk_nomor = h.min_spk_nomor
        LEFT JOIN kencanaprintnew.tmemospk m ON m.mspk_nomor = h.min_spk_nomor
        LEFT JOIN kencanaprintnew.tpabrik p ON p.pab_kode = h.min_cab
        WHERE h.min_tanggal >= ? AND h.min_tanggal <= ? 
          AND h.min_jenis IN ('OBAT', 'ACCESORIES')
          AND h.min_cab = 'P03'
          ${searchFilter}
      ) x 
      LEFT JOIN kencanaprintnew.tdivisi v ON v.kode = x.kddiv
...
const getDetails = async (req, res) => {
  try {
    ...
    const [realisasiRows] = await pool.query(
      ...
      FROM kencanaprintnew.tgarmenrealisasi_hdr h
      INNER JOIN kencanaprintnew.tgarmenrealisasi_dtl d ON d.red_nomor = h.re_nomor
      WHERE h.re_minta = ?
      GROUP BY h.re_nomor
    ...
    const [itemsRows] = await pool.query(
      ...
      SELECT 
        ...
        IFNULL((
          ...
          FROM kencanaprintnew.tgarmenrealisasi_dtl i 
          INNER JOIN kencanaprintnew.tgarmenrealisasi_hdr j ON j.re_nomor = i.red_nomor 
    ...
    const [realisasiDetailsRows] = await pool.query(
      ...
      FROM kencanaprintnew.tgarmenrealisasi_dtl d
      INNER JOIN kencanaprintnew.tgarmenrealisasi_hdr h ON h.re_nomor = d.red_nomor
      LEFT JOIN kencanaprintnew.tgarmen_brg b ON b.brg_kode = d.red_brg_kode
      WHERE h.re_minta = ?
...
const checkUnapproved = async (req, res) => {
  try {
    ...
    const query = `
      ...
      FROM kencanaprintnew.tgarmenrealisasi_hdr h
      INNER JOIN kencanaprintnew.tgarmenminta_hdr a ON a.min_nomor = h.re_minta AND a.user_create = ?
...
const approveRealisasi = async (req, res) => {
  ...
  try {
    ...
    const qCek = `
      ...
      FROM kencanaprintnew.tgarmenrealisasi_hdr h
      INNER JOIN kencanaprintnew.tgarmenminta_hdr m ON m.min_nomor = h.re_minta
      WHERE h.re_nomor = ?
    `;
    ...
    await conn.query(
      `UPDATE kencanaprintnew.tgarmenrealisasi_hdr SET re_apv = ? WHERE re_nomor = ?`,
    ...
    if (["ACCESORIES", "OBAT"].includes(data.min_jenis)) {
      const [detailRows] = await conn.query(
        `SELECT red_brg_kode AS brg_kode, red_jumlah AS jumlah
         FROM kencanaprintnew.tgarmenrealisasi_dtl
         WHERE red_nomor = ?`,
        [noRealisasi],
      );
