    ...
    try {
        ...
        if (isNew) {
            ...
            await connection.query(
                ...
                [sjNomor, header.tanggal, header.store.kode, header.permintaan, header.keterangan, user.kode]
            );
        } else {
            await connection.query(
                `UPDATE tdc_sj_hdr SET sj_tanggal = ?, sj_kecab = ?, sj_ket = ?, user_modified = ?, date_modified = NOW() WHERE sj_nomor = ?;`,
                [header.tanggal, header.store.kode, header.keterangan, user.kode, sjNomor]
            );
        }
        ...
        res.status(201).json({ success: true, message: `Surat Jalan ${sjNomor} berhasil disimpan.`, data: { nomor: sjNomor } });
    } catch (error) {
...
const getItemsForLoad = async (req, res) => {
    try {
        ...
        if (!nomor || !gudang) {
            return res.status(400).json({ success: false, message: 'Parameter "nomor" dan "gudang" diperlukan.' });
        }
...
const loadForEdit = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, data: { header: headerRows[0], items } });
    } catch (error) {
...
const searchStores = async (req, res) => {
    try {
        ...
        const [[{ total }]] = await pool.query(`SELECT COUNT(*) as total FROM tgudang ${whereClause}`, params);
        
        const [items] = await pool.query(`SELECT gdg_kode AS kode, gdg_nama AS nama FROM tgudang ${whereClause} ORDER BY gdg_kode LIMIT ? OFFSET ?;`, [...params, parseInt(itemsPerPage), parseInt(offset)]);
...
const searchPermintaan = async (req, res) => {
    try {
        ...
        if (!storeKode) {
            return res.status(400).json({ success: false, message: 'Parameter "storeKode" diperlukan.' });
        }
        const offset = (page - 1) * itemsPerPage;
        const searchTerm = `%${term || ''}%`;
        ...
        const [items] = await pool.query(`SELECT h.mt_nomor AS nomor, h.mt_tanggal AS tanggal, h.mt_otomatis AS otomatis, h.mt_ket AS keterangan ${baseQuery} ORDER BY h.date_create DESC LIMIT ? OFFSET ?;`, [...params, parseInt(itemsPerPage), parseInt(offset)]);
...
const searchTerimaRb = async (req, res) => {
    try {
        ...
        const [[{ total }]] = await pool.query(`SELECT COUNT(*) AS total ${baseQuery}`, params);
        const [items] = await pool.query(`SELECT h.rb_nomor AS nomor, h.rb_tanggal AS tanggal, r.rb_nomor AS no_rb, r.rb_tanggal AS tgl_rb, CONCAT(LEFT(r.rb_nomor, 3), ' - ', g.gdg_nama) AS dari_store ${baseQuery} ORDER BY h.date_create DESC LIMIT ? OFFSET ?;`, [...params, parseInt(itemsPerPage), parseInt(offset)]);
