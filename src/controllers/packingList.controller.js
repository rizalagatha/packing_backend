...
const searchPermintaanOpen = async (req, res) => {
  ...
  try {
    ...
    if (storeKode) {
      whereClause += " AND h.mt_gdg_kode = ?";
      params.push(storeKode);
    }

    // Filter by Search Term (Nomor Permintaan)
    if (term) {
      whereClause += " AND h.mt_nomor LIKE ?";
      params.push(`%${term}%`);
    }
    ...
    const query = `
        SELECT 
            h.mt_nomor, 
            h.mt_tanggal,
            COUNT(d.mtd_kode) as total_items
